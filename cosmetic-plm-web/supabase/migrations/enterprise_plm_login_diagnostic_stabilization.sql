-- Enterprise PLM Login Diagnostic Stabilization
-- Correct admin email: kdw0702s2dw@gmail.com
...
select
  ...
  case when lower(u.email) = 'kdw0702s2dw@gmail.com' then 'Admin' else 'Researcher' end,
  true
from auth.users u
where u.email is not null
on conflict (id) do update set
  email = excluded.email,
  role = case when lower(excluded.email) = 'kdw0702s2dw@gmail.com' then 'Admin' else public.plm_user_profiles.role end,
  is_active = true,
  updated_at = now();

update public.plm_user_profiles
set role = 'Admin', is_active = true, updated_at = now()
where lower(email) = 'kdw0702s2dw@gmail.com';
...
values
('LOGIN-DIAGNOSTIC-STABILIZATION', 'Enterprise PLM Login Diagnostic Stabilization', 'Shows exact login errors and sets correct admin email kdw0702s2dw@gmail.com.')
