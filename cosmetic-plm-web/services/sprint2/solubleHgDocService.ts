...
import type { SolubleHgSheet } from "@/services/sprint2/solubleHgService";
...
function lossRateLabel(sheet: SolubleHgSheet) {
...
function solubleHgMeta(sheet: SolubleHgSheet) {
  return {
...
export function buildSolubleHgHtml(sheet: SolubleHgSheet): string {
  const meta = solubleHgMeta(sheet);
  const metaRows = Object.entries(meta)
    .map(([k, v]) => `<tr><td class="k">${escapeHtml(k)}</td><td class="colon">:</td><td class="v">${escapeHtml(v)}</td></tr>`)
    .join("");

  const inputRows = [
    [`관리기준1 (${sheet.component1_raw_code || "-"})`, fmt(sheet.component1_weight)],
    [`관리기준2 (${sheet.component2_raw_code || "-"})`, fmt(sheet.component2_weight)],
    [`관리기준3 (${sheet.component3_raw_code || "-"})`, fmt(sheet.component3_weight)],
  ...
  const resultRows = [
    ...
    ["성형품 최소 중량(부자재제외)", fmt(sheet.loss_adjusted_coat_amount)],
    ["성형품 최대 중량(부자재제외)", fmt(sheet.standard_coating_amount)],
    ...
    ["최소 기준 합계(부자재포함)", fmt(sheet.min_total_weight)],
    ["최대 기준 합계(부자재포함)", fmt(sheet.max_total_weight)],
  ]
    .map(([k, v]) => `<tr><td>${escapeHtml(k)}</td><td class="right">${escapeHtml(v)}</td></tr>`)
    .join("");
...
<body>
<div class="page">
<div class="doctitle">수용성 HG 도포량 계산</div>
...
export function printSolubleHgSheet(sheet: SolubleHgSheet) {
  openPrintDocument({ html_content: buildSolubleHgHtml(sheet) });
}

export async function downloadSolubleHgExcel(sheet: SolubleHgSheet) {
  const wb = new ExcelJS.Workbook();
  const ws = wb.addWorksheet("수용성 HG");
  const colCount = 4;
  ws.columns = [{ width: 24 }, { width: 18 }, { width: 16 }, { width: 16 }];

  writeTitleRow(ws, "수용성 HG 도포량 계산", colCount);
  writeMetaRows(ws, solubleHgMeta(sheet), colCount);
  ...
  const inputPairs: [string, number | string | null][] = [
    [`관리기준1 (${sheet.component1_raw_code || "-"})`, sheet.component1_weight],
    [`관리기준2 (${sheet.component2_raw_code || "-"})`, sheet.component2_weight],
    [`관리기준3 (${sheet.component3_raw_code || "-"})`, sheet.component3_weight],
  ...
  const resultPairs: [string, number | null | undefined][] = [
    ...
    ["성형품 최소 중량(부자재제외)", sheet.loss_adjusted_coat_amount],
    ["성형품 최대 중량(부자재제외)", sheet.standard_coating_amount],
    ...
    ["최소 기준 합계(부자재포함)", sheet.min_total_weight],
    ["최대 기준 합계(부자재포함)", sheet.max_total_weight],
  ];
  ...
  await downloadWorkbook(wb, `수용성HG_${sheet.formula_code}_${sheet.revision}.xlsx`);
}
